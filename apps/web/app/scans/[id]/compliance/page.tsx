...
const TARGET_LABEL: Record<string, string> = {
  ...
  llm: "LLM endpoint target",
};

export default function ScanCompliancePage() {
  ...
  useEffect(() => {
    if (!scanId) return;
    setError(null);
    api<ComplianceRollup>(`/scans/${scanId}/compliance`)
      .then((d) => {
        setData(d);
    ...
    <main className="mx-auto max-w-6xl px-6 py-10">
      ...
      <div className="mt-4 flex items-baseline justify-between gap-4 flex-wrap">
        ...
        <span className="font-mono text-xs uppercase text-neutral-500">
          {TARGET_LABEL[data.target_kind] ?? data.target_kind}
        </span>
      </div>

      <p className="mt-3 text-sm text-neutral-700 max-w-[72ch]">
        Every active finding on this assessment, fanned out across the
        frameworks that match the target&rsquo;s asset class. The same rollup is
        rendered into the DOCX / Markdown report&rsquo;s compliance appendix and
        into JSON / CSV exports.
      </p>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <SummaryStat label="Target" value={data.target_kind.toUpperCase()} />
      </div>
      ...
      {fw && summary ? (
        <FrameworkPanel scanId={scanId} framework={fw} summary={summary} />
      ) : (
        <p className="mt-6 text-sm text-neutral-500">
          No frameworks enabled for this target.
        </p>
      )}

      <FindingsTable scanId={scanId} findings={data.findings} />
    </main>
  );
}
      ...
      <table className="w-full text-sm">
        <thead className="bg-neutral-50 text-left">
          <tr>
            ...
            <th className="px-3 py-2">Open finding</th>
          </tr>
        </thead>
        <tbody>
          {summary.controls.map((c) => (
            <tr key={c.id} className="border-t border-neutral-200 align-top">
              ...
              <td className="px-3 py-2">
                {c.finding_ids.slice(0, 3).map((fid) => (
                  <Link
                    key={fid}
                    href={`/scans/${scanId}/findings/${fid}`}
                    className="block font-mono text-xs underline underline-offset-2 hover:opacity-70"
                  >
                    {fid.slice(0, 8)}
                  </Link>
                ))}
                {c.finding_ids.length > 3 ? (
                  <span className="block text-xs text-neutral-500">
                    +{c.finding_ids.length - 3} more
                  </span>
                ) : null}
              </td>
            </tr>
  ...
  return (
    <section className="mt-12">
      ...
      <p className="mt-1 text-sm text-neutral-600">
        Each row shows the canonical finding plus the controls it triggers
        across every enabled framework.
      </p>
